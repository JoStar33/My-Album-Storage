...
const ScoreDialog: React.FC<propsType> = ({scoreDialog, selectedAlbums, setScoreDialog, setSelectedAlbums}) => {
  ...
  const handleChangeScore = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setScoreDialog({...scoreDialog, scoreAlbum: {
  ...
  const applyScore = () => {
    ...
    //선택이 되었음을 알리는 함수 호출
    dispatch(setIsSelected({key: scoreDialog.scoreAlbum.key, isSelected: true}));
    //선택이 됐었던 앨범이 아니라면? push진행
    !selectedAlbums
    .find(selectedAlbum => selectedAlbum.key === scoreDialog.scoreAlbum.key) ? pushScoreAlbum() : updateSeletedAlbum()
  };
  ...
  const updateSeletedAlbum = () => {
    ...
    .map(selectedAlbum => selectedAlbum.key === scoreDialog.scoreAlbum.key ? scoreDialog.scoreAlbum : selectedAlbum);
  ...
  return (
    <DialogBackground>
      <ScoreDialogContainer>
        <AlbumTitle>
          {scoreDialog.scoreAlbum.name}
        </AlbumTitle>
        <AlbumImg src={scoreDialog.scoreAlbum.image}></AlbumImg>
        <ArtistName>
          {scoreDialog.scoreAlbum.artist}
        </ArtistName>
